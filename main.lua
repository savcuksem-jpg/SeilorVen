...
getgenv().SeilorVen_Running = true
...
local function PostToWebhook()
    ...
    local payload = {
        ["embeds"] = {{
            ...
            ["footer"] = { ["text"] = string.format("SeilorVen • Session: %s • %s", GetSessionTime(), os.date("%x %X")) },
...
local function SendSafetyWebhook(targetPlayer, reason)
    ...
    local payload = {
        ["embeds"] = {{
            ...
            ["footer"] = { ["text"] = "SeilorVen • " .. os.date("%x %X") }
        }}
    }
...
task.spawn(function()
    while task.wait(5) do
        if not getgenv().SeilorVen_Running then break end
...
-- Discord server sections removed
...
task.spawn(function()
    while getgenv().SeilorVen_Running do
...
MenuGroup:AddButton("Unload", function()
    getgenv().SeilorVen_Running = false
    Shared.Farm = false
    Cleanup(Connections)
    Cleanup(Flags)
	Library:Unload()
...
ThemeManager:SetFolder("SeilorVen")
SaveManager:SetFolder("SeilorVen/SailorPiece")
...
-- bug report notification removed
